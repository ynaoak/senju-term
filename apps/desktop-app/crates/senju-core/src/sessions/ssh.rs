...
use super::forward::{parse_forward_spec, ForwardSpec};
...
/// `SessionManager` can register the session before any `data`/`exit` event
/// can fire. See [`super::local::LocalReader`] for why that ordering matters
/// (the reader is what removes the session from the map, so running it first
/// would strand the entry there permanently).
pub(crate) struct SshReader {
    read_half: russh::ChannelReadHalf,
}

impl SshReader {
    /// Pumps channel output to the sink until the channel closes, then drops
    /// the session from the map and announces the exit. Call only after the
    ...
    pub fn start(self, id: String, sink: Arc<dyn EventSink>, sessions: SessionMap) {
        ...
        tokio::spawn(async move {
            ...
            while let Some(msg) = read_half.wait().await {
                match msg {
                    ChannelMsg::Data { data } => sink.data(&id, &data),
                    ChannelMsg::ExtendedData { data, .. } => sink.data(&id, &data),
...
pub(crate) struct SshSession {
    ...
    /// Listener tasks for this session's local port forwards. Aborted on
    /// drop, so forwards stop both on explicit kill and when the channel
    /// closes on its own (the reader task removes the session from the map,
    /// dropping this struct).
    forward_tasks: Vec<tokio::task::JoinHandle<()>>,
    /// Intermediate jump-host handles for a multi-hop connection, kept alive
    /// so their tunnels stay open for the session's lifetime. Dropping them
    /// (on session close) tears the tunnels down; the target handle in
    /// `writer` drops first (declared earlier), closing the shell channel
...
/// using the details `Client::check_server_key` stashed along the way plus the
/// hop identity (`host_id`/`host`/`port`) so a multi-hop trust prompt can name
/// and re-approve exactly the hop that was unknown.
fn host_key_error(
    e: russh::Error,
    outcome: &StdMutex<Option<HostKeyOutcome>>,
    host_id: &str,
    host: &str,
    port: u16,
) -> SessionError {
...
/// One resolved hop in a connection chain: the host to reach, the secrets to
/// authenticate with, and (for a re-connect after a TOFU prompt) the
/// fingerprint the user approved for this exact host.
...
/// A hop's transport: the first hop dials a real TCP socket; every later hop
/// rides a direct-tcpip channel opened on the previous hop (nested tunnels —
/// this is what makes multi-hop / ProxyJump work).
enum HopTransport {
    Tcp,
    /// Boxed: a channel stream is far larger than the unit `Tcp` variant, so
    /// storing it inline would bloat every `HopTransport`.
    Stream(Box<ChannelStream<client::Msg>>),
}
...
/// Connects to and authenticates one hop, verifying its host key against
/// known_hosts (same fail-closed TOFU rules as a direct connect). Returns the
/// live `Handle` on success.
async fn establish_hop(
    ...
    let handler = Client {
        ...
        port: host.port,
        expected_fingerprint: hop.expected_fingerprint.clone(),
        known_hosts_path,
    ...
    let connect = async {
        match transport {
            HopTransport::Tcp => {
                client::connect(ssh_config(), (host.host.as_str(), host.port), handler).await
            }
            HopTransport::Stream(stream) => {
                client::connect_stream(ssh_config(), *stream, handler).await
            }
        }
    };

    let mut handle = match tokio::time::timeout(SSH_STAGE_TIMEOUT, connect).await {
        Ok(res) => res.map_err(|e| host_key_error(e, &outcome, &host.id, &host.host, host.port))?,
        Err(_) => return Err(SessionError::Ssh("接続がタイムアウトしました".into())),
    };

    match tokio::time::timeout(SSH_STAGE_TIMEOUT, authenticate(&mut handle, host, &hop.secrets)).await
    {
        Ok(res) => res?,
...
impl SshSession {
    /// Direct (single-hop) connect. Connects, authenticates, opens a shell
    /// channel and starts the host's local port forwards. Returns the session,
    /// the not-yet-running [`SshReader`] — see that type for why output
    /// pumping only begins once the caller has registered the session — and
    /// any port-forward banner lines to show in the terminal.
    pub async fn connect(
        ...
        expected_fingerprint: Option<String>,
        known_hosts_path: PathBuf,
    ) -> Result<(Self, SshReader, Vec<String>), SessionError> {
        let hop = ChainHop { host: host.clone(), secrets, expected_fingerprint };
        let handle = establish_hop(&hop, known_hosts_path, HopTransport::Tcp).await?;
        Self::finish_shell_session(handle, Vec::new(), host, cols, rows).await
    }

    /// Multi-hop connect (ProxyJump). `chain` is ordered `[jump1, .., jumpN,
    /// target]`; each hop after the first is reached over a direct-tcpip
    /// tunnel opened on the previous hop. Intermediate `Handle`s are kept
    /// alive for the session's lifetime so the tunnels stay open.
    pub async fn connect_chain(
        ...
        assert!(!chain.is_empty(), "connect_chain requires at least the target");
        let mut handles: Vec<Handle<Client>> = Vec::with_capacity(chain.len());
        for (i, hop) in chain.iter().enumerate() {
            let transport = if i == 0 {
                HopTransport::Tcp
            } else {
                // Open the next hop's tunnel through the hop we just built.
                let prev = &handles[i - 1];
                let channel = prev
                    .channel_open_direct_tcpip(hop.host.host.as_str(), hop.host.port as u32, "127.0.0.1", 0)
                    .await
                    .map_err(|e| {
                        SessionError::Ssh(format!(
                            "踏み台 {} 経由のトンネルを開けません: {e}",
                            chain[i - 1].host.name.as_str()
                        ))
                    })?;
                HopTransport::Stream(Box::new(channel.into_stream()))
            };
            let handle = establish_hop(hop, known_hosts_path.clone(), transport).await?;
            handles.push(handle);
        }
        // Last handle is the target; everything before it are the jump hops.
        let target_handle = handles.pop().expect("chain non-empty");
        let target = &chain.last().expect("chain non-empty").host;
        Self::finish_shell_session(target_handle, handles, target, cols, rows).await
    }

    /// Shared tail: opens the shell channel on the (already authenticated)
    /// target handle, starts local port forwards, and builds the session.
    /// `jump_handles` are the intermediate hops to keep alive (empty for a
    /// direct connect).
    async fn finish_shell_session(
        handle: Handle<Client>,
        jump_handles: Vec<Handle<Client>>,
        host: &SshHost,
        cols: u16,
        ...
        let writer = Arc::new(AsyncMutex::new(SshWriter { write_half, handle }));

        // Local port forwards (ssh -L). Failures never fail the session —
        // they are reported as banner lines in the terminal, mirroring how
        // OpenSSH complains but keeps the shell.
        //
        // The lines are returned rather than written to the sink here: at this
        // point the session is not yet in the map and the frontend does not
        // even know its id, so `SessionManager` emits them once it has
        // registered the session (see `emit_banners`).
        let mut banners = Vec::new();
        let mut forward_tasks = Vec::new();
        for raw in host.forwards.iter().filter(|s| !s.trim().is_empty()) {
            let spec = match parse_forward_spec(raw) {
                Ok(spec) => spec,
                Err(err) => {
                    banners.push(format!("\x1b[31m[senju] ポートフォワード設定が不正です: {err}\x1b[0m"));
                    continue;
                }
            };
            match tokio::net::TcpListener::bind(("127.0.0.1", spec.local_port)).await {
                Ok(listener) => {
                    banners.push(format!(
                        "\x1b[36m[senju] ポートフォワード 127.0.0.1:{} → {}:{}\x1b[0m",
                        spec.local_port, spec.remote_host, spec.remote_port
                    ));
                    forward_tasks.push(tokio::spawn(run_local_forward(writer.clone(), spec, listener)));
                }
                Err(e) => banners.push(format!(
                    "\x1b[31m[senju] ポートフォワード {spec} を開始できません: {e}\x1b[0m"
                )),
            }
        }
    ...
    pub fn kill(self) {
        // Clone rather than move: SshSession has a Drop impl (it aborts the
        // port-forward listener tasks), so fields can't be moved out of self.
        ...
        // self drops here → Drop aborts the forward listeners.
    }
}

/// Writes app-generated status lines (the port-forward banners from
/// [`SshSession::connect`]) into the session's terminal stream. Called by
/// `SessionManager` after the session is in the map, so the lines land ahead
/// of the shell's own first output.
pub(crate) fn emit_banners(sink: &Arc<dyn EventSink>, id: &str, lines: &[String]) {
    for line in lines {
        sink.data(id, format!("\r\n{line}\r\n").as_bytes());
    }
}

/// Accept loop for one `-L` forward: each local TCP connection gets its own
/// direct-tcpip channel through the SSH session, pumped bidirectionally until
/// either side closes. A failed channel open (e.g. the remote port refused
/// the connection) just drops the local socket, like OpenSSH.
async fn run_local_forward(
    ...
    loop {
        ...
        let spec = spec.clone();
        tokio::spawn(async move {
            // Hold the writer lock only for the channel-open handshake, not
...
pub async fn test_connection(
    ...
    let mut handle = match tokio::time::timeout(
        SSH_STAGE_TIMEOUT,
        client::connect(ssh_config(), (host.host.as_str(), host.port), handler),
    )
    .await
    {
        Ok(res) => res.map_err(|e| host_key_error(e, &outcome, &host.id, &host.host, host.port))?,
