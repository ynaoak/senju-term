...
pub enum SessionError {
    ...
    /// message is machine-parseable by the frontend (offer a "trust and save"
    /// prompt): fields are `\u{1f}`-delimited (a byte that can't occur in a
    /// hostname/fingerprint, so it's a safe separator even for IPv6 hosts) and
    /// led by the `UNKNOWN_HOST_KEY` tag. `host_id` keys the per-hop trust
    /// approval for multi-hop (ProxyJump) connects — it identifies exactly
    /// which hop in the chain is unknown; `host`/`port` are for display.
    #[error("UNKNOWN_HOST_KEY\u{1f}{host_id}\u{1f}{host}\u{1f}{port}\u{1f}{key_type}\u{1f}{fingerprint}")]
    UnknownHostKey {
        host_id: String,
        host: String,
        port: u16,
    ...
    #[error("HOST_KEY_MISMATCH\u{1f}{host_id}\u{1f}{host}\u{1f}{port}\u{1f}{line}\u{1f}{key_type}\u{1f}{fingerprint}")]
    HostKeyMismatch {
        host_id: String,
        host: String,
        port: u16,
    ...
    /// no fallback that accepts it.
    #[error("FINGERPRINT_MISMATCH\u{1f}{host_id}\u{1f}{host}\u{1f}{port}\u{1f}{key_type}\u{1f}{expected}\u{1f}{actual}")]
    FingerprintMismatch {
        host_id: String,
        host: String,
        port: u16,
...
impl SessionManager {
    ...
    pub fn create_local(
        ...
        let (session, title, reader) =
            local::LocalSession::spawn(spec, cols, rows, shell_integration)?;
        // Register first, pump second. The reader is what removes the session
        // on EOF, so starting it before the insert lets a shell that dies
        // immediately have its removal run against a map that doesn't contain
        // it yet — leaving an entry nothing will ever reap. See `LocalReader`.
        self.sessions
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .insert(id.clone(), Backend::Local(session));
        reader.start(id.clone(), self.sink.clone(), self.sessions.clone());
        Ok(SessionInfo {
            id,
            title,
            kind: "local".into(),
    ...
    pub async fn create_ssh(
        ...
        let (session, reader, banners) = ssh::SshSession::connect(
            host,
            secrets,
            cols,
            rows,
        ...
        .await?;
        // Register before pumping, for the same reason as `create_local`: the
        // reader removes the session when the channel closes, and a server that
        // drops the shell straight away would otherwise strand the entry.
        self.sessions
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .insert(id.clone(), Backend::Ssh(session));
        // Port-forward status lines first, so they precede the shell's banner
        // rather than being interleaved into it by the reader.
        ssh::emit_banners(&self.sink, &id, &banners);
        reader.start(id.clone(), self.sink.clone(), self.sessions.clone());
        Ok(SessionInfo {
            id,
    ...
    /// Connects to the last host in `chain` *through* the earlier ones
    /// (ProxyJump / multi-hop). `chain` must be the ordered
    /// `[jump1, .., jumpN, target]` produced by [`resolve_jump_chain`].
    /// `secrets_by_id` / `fingerprints_by_id` are keyed by each hop's saved
    /// host id: secrets supply per-hop credentials (missing = none, e.g. an
    /// agent hop), and fingerprints carry the SHA256 the user approved for a
    /// specific hop during a prior TOFU prompt (so re-connecting trusts only
    /// that exact hop's exact key).
    pub async fn create_ssh_chain(
        &self,
        chain: &[SshHost],
        secrets_by_id: &std::collections::HashMap<String, SshSecrets>,
        cols: u16,
        rows: u16,
        fingerprints_by_id: &std::collections::HashMap<String, String>,
    ) -> Result<SessionInfo, SessionError> {
        let target = chain
            .last()
            .ok_or_else(|| SessionError::Ssh("接続チェーンが空です".into()))?
            .clone();
        let hops: Vec<ssh::ChainHop> = chain
            .iter()
            .map(|h| ssh::ChainHop {
                host: h.clone(),
                secrets: secrets_by_id.get(&h.id).cloned().unwrap_or_default(),
                expected_fingerprint: fingerprints_by_id.get(&h.id).cloned(),
            })
            .collect();
        let id = uuid::Uuid::new_v4().to_string();
        let known_hosts_path = self
            ...
            .clone()
            .unwrap_or_else(ssh::default_known_hosts_path);
        let (session, reader, banners) =
            ssh::SshSession::connect_chain(hops, cols, rows, known_hosts_path).await?;
        // Register before pumping, same ordering rule as `create_ssh`.
        self.sessions
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .insert(id.clone(), Backend::Ssh(session));
...
mod tests {
    ...
    /// Same end-to-end check for zsh — the default shell on macOS, and the one
    /// whose integration works by pointing `ZDOTDIR` at generated proxy rc
    /// files. The unit tests in `shell_integration` only inspect those files'
    /// text; this proves zsh actually reads them and emits the markers.
    #[tokio::test(flavor = "multi_thread")]
    async fn shell_integration_makes_zsh_emit_osc_133() {
        if !std::path::Path::new("/bin/zsh").exists() {
            return; // no zsh on this machine
        }
        let sink = Arc::new(Capture::default());
        let mgr = SessionManager::new(sink.clone());
        let spec = LocalSpec { command: "/bin/zsh".into(), ..Default::default() };
        let info = mgr.create_local(&spec, 80, 24, true).unwrap();
    ...
    /// Q5: a shell that exits the instant it starts must still end up removed
    /// from the session map. The reader performs that removal, so starting it
    /// before the insert (as the code used to) let the removal run against a
    /// map that didn't hold the session yet, stranding an entry nothing would
    /// ever reap — it kept the PTY, the child handle and the integration temp
    /// dir alive, and the frontend had already dropped its thread on `exit`.
    /// A session that is really gone reports `UnknownSession`; a stranded one
    /// would instead be found and fail (or succeed) down at the PTY layer.
    ///
    /// Note this asserts the end-state invariant, not the ordering: the
    /// ordering itself is now enforced structurally, since `LocalReader` can
    /// only pump once `start()` is called and the sole caller does that after
    /// the insert. Losing the race again would make this test flaky, not
    /// reliably red.
    #[tokio::test(flavor = "multi_thread")]
    async fn instantly_exiting_shell_is_not_stranded_in_the_session_map() {
        let sink = Arc::new(Capture::default());
        let mgr = SessionManager::new(sink.clone());
        // `sh -c exit` is gone by the time the spawn call returns.
        let spec = LocalSpec {
            command: "/bin/sh".into(),
            args: vec!["-c".into(), "exit 7".into()],
            ..Default::default()
        };
        let info = mgr.create_local(&spec, 80, 24, false).unwrap();

        assert!(
            wait_until(Duration::from_secs(10), || sink.exited.lock().unwrap().is_some()),
            "the shell should have exited and reported it"
        );
        assert!(
            matches!(
            ...
            "an exited session must be gone from the map, not stranded in it"
        );
    }
